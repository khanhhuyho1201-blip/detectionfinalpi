...
def parse_line(line: str, st: MachineStatus) -> tuple[MachineStatus, str]:
    ...
    if upper.startswith("[NOMOTOR]"):
        # v6.4: firmware applied PWM but the encoder never turned -> motor has no
        # power / is jammed. Distinct from STALL (motor spins but no cards).
        s.error = ERR_NOMOVE
        s.state = ERROR
        return s, "nomotor"
